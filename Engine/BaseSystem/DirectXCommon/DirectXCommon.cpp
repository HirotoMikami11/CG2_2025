...
void DirectXCommon::MakeOffscreenPSO()
{
	...
	//バイナリを元に生成
	hr = device->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
		signatureBlob->GetBufferSize(), IID_PPV_ARGS(&offscreenRootSignature));
	assert(SUCCEEDED(hr));
	...
	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = CompileShader(L"resources/Shader/Offscreen.PS.hlsl",
		L"ps_6_0", dxcUtils, dxcCompiler, includeHandler);
	assert(pixelShaderBlob != nullptr);
	...
	D3D12_GRAPHICS_PIPELINE_STATE_DESC spritePipelineStateDesc{};
	spritePipelineStateDesc.pRootSignature = offscreenRootSignature.Get();
	spritePipelineStateDesc.InputLayout = inputLayoutDesc;
	spritePipelineStateDesc.VS = { vertexShaderBlob->GetBufferPointer(), vertexShaderBlob->GetBufferSize() };
	spritePipelineStateDesc.PS = { pixelShaderBlob->GetBufferPointer(), pixelShaderBlob->GetBufferSize() };
	spritePipelineStateDesc.BlendState = blendDesc;
	...
	// 実際に生成
	hr = device->CreateGraphicsPipelineState(&spritePipelineStateDesc,
		IID_PPV_ARGS(&offscreenPipelineState));
	assert(SUCCEEDED(hr));
	//生成できなかったら起動できない
	Logger::Log(Logger::GetStream(), "Complete create offscreenPSO!!\n");//PSO生成完了のログを出す
