...
void GlitchEffect::Initialize(DirectXCommon* dxCommon) {
    ...
    // 初期プリセットを適用
    ApplyPreset(EffectPreset::SUBTLE);
...
void GlitchEffect::CreateConstantBuffer() {
    ...
    // 各メンバーのオフセットを確認
    Logger::Log(Logger::GetStream(), std::format("color offset: {}\n", offsetof(GlitchMaterialData, color)));
    Logger::Log(Logger::GetStream(), std::format("enableLighting offset: {}\n", offsetof(GlitchMaterialData, enableLighting)));
    Logger::Log(Logger::GetStream(), std::format("uvTransform offset: {}\n", offsetof(GlitchMaterialData, uvTransform)));
    Logger::Log(Logger::GetStream(), std::format("rgbShiftStrength offset: {}\n", offsetof(GlitchMaterialData, rgbShiftStrength)));
    Logger::Log(Logger::GetStream(), std::format("time offset: {}\n", offsetof(GlitchMaterialData, time)));
    Logger::Log(Logger::GetStream(), std::format("glitchIntensity offset: {}\n", offsetof(GlitchMaterialData, glitchIntensity)));
...
void GlitchEffect::ImGui() {
    if (ImGui::CollapsingHeader("Glitch Effect")) {
        ...
        if (isEnabled_) {
            ...
            // プリセット選択
            const char* presetNames[] = { "Off", "Subtle", "Medium", "Intense", "Chaos" };
            if (ImGui::Combo("Preset", &currentPreset_, presetNames, IM_ARRAYSIZE(presetNames))) {
                ApplyPreset(static_cast<EffectPreset>(currentPreset_));
            }
...
void GlitchEffect::ApplyPreset(EffectPreset preset) {
    ...
    case EffectPreset::SUBTLE:
        isEnabled_ = true;
        materialData_.rgbShiftStrength = 5.0f;  // 0.5f → 5.0f に増加
        materialData_.glitchIntensity = 1.0f;   // 0.2f → 1.0f に増加
        animationSpeed_ = 1.0f;
        break;

    case EffectPreset::MEDIUM:
        isEnabled_ = true;
        materialData_.rgbShiftStrength = 10.0f; // 2.0f → 10.0f に増加
        materialData_.glitchIntensity = 1.0f;   // 0.5f → 1.0f に増加
        animationSpeed_ = 1.5f;
        break;

    case EffectPreset::INTENSE:
        isEnabled_ = true;
        materialData_.rgbShiftStrength = 4.0f;
        materialData_.glitchIntensity = 0.8f;
        animationSpeed_ = 2.0f;
        break;

    case EffectPreset::CHAOS:
        isEnabled_ = true;
        materialData_.rgbShiftStrength = 8.0f;
        materialData_.glitchIntensity = 1.0f;
        animationSpeed_ = 3.0f;
        break;
    }

    currentPreset_ = static_cast<int>(preset);
    UpdateConstantBuffer();
}
